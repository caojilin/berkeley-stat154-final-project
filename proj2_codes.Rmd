...
ggplot(data = image1) + geom_point(aes(x = x_coordinate, y = y_coordinate, color = as.factor(expert_label))) + scale_y_reverse() + scale_color_manual("Expert Labels", labels = c("Not Cloud", "No Label", "Cloud"), values = c("grey", "black", "white")) + ggtitle('Image 1: ')
...
#2 a)
```{r}
# Non-Trivial Split Method 1: Split each image into four squares, extract 80% training, and 20% testing from each square
...
```{r}
# Non-trivial Method 2: split each image into many smaller squares and randomly select some squares to be testing and others to be training 
range(image1_nozero$y_coordinate)
range(image1_nozero$x_coordinate)
range(image2_nozero$y_coordinate)
range(image2_nozero$x_coordinate)
range(image3_nozero$y_coordinate)
range(image3_nozero$x_coordinate)
...
testM2 <- rbind(image1_all$test,
                    image2_all$test,
                    image3_all$test)
testM2 <- unique(testM2)

train_validationM2 <- unique(rbind(trainM2,validationM2))
...
```{r}
# d)
CVgeneric <- function(classifier, features, labels, K=5, loss_fn, n_tree=10, knn_k=10) {
  ...
  for (i in 1: K) {
    ...
    mod_fit = train(train.feature, train.label , method = classifier)
    
    y_hat = predict(mod_fit, newdata=test.feature)
    loss[i] = loss_fn(y_hat, test.label)
    accuracy[i] = mean(y_hat == test.label)
    }
  return(output)
}
```
