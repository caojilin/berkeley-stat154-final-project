...
```{r}
# Method 3: Naive Split

test_image1 <- image1_nozero[1:floor(nrow(image1_nozero) * 0.2),]
test_image2 <- image2_nozero[1:floor(nrow(image1_nozero) * 0.2),]
test_image3 <- image3_nozero[1:floor(nrow(image1_nozero) * 0.2),]
val_train_image1 <- image1_nozero[(floor(nrow(image1_nozero) * 0.2) + 1):nrow(image1_nozero), ]
val_train_image2 <- image2_nozero[(floor(nrow(image2_nozero) * 0.2) + 1):nrow(image2_nozero), ]
val_train_image3 <- image3_nozero[(floor(nrow(image3_nozero) * 0.2) + 1):nrow(image3_nozero), ]
val_image1 <- image1_nozero[floor(nrow(val_train_image1) * 0.2), ]
val_image2 <- image2_nozero[floor(nrow(val_train_image2) * 0.2), ]
val_image3 <- image3_nozero[floor(nrow(val_train_image3) * 0.2), ]
train_image1 <- image1_nozero[(floor(nrow(val_train_image1) * 0.2) + 1):nrow(val_train_image1), ]
train_image2 <- image2_nozero[(floor(nrow(val_train_image2) * 0.2) + 1):nrow(val_train_image2), ]
train_image3 <- image3_nozero[(floor(nrow(val_train_image3) * 0.2) + 1):nrow(val_train_image3), ]

trainM2 <- rbind(train_image1, 
               train_image2, train_image3)
validationM2 <- rbind(val_image1,
                    val_image2,
                    val_image3)

testM2 <- rbind(test_image1,
                    test_image2,
                    test_image3)
...
```
```{r}
# Method 4: Even more naive split
...
train_validationM2 <- rbind(val_train_image1, val_train_image2, val_train_image3)
testM2 <- rbind(test_image1, test_image2, test_image3)
```
...
CV(train_validation, 10)

# Split 2
CV(train_validationM2, 10)


#test
mod_fit <- glm(expert_label ~ SD + CORR + NDAI,
                       data = val_train_image1,
                       family = "binomial")
y_hat = predict(mod_fit, newdata=test_image1)
      y_hat <- y_hat > 0.5
      y_hat[which(y_hat == FALSE)] <- 0
      y_hat[which(y_hat == TRUE)] <- 1
      mean(y_hat  == test_image1$expert_label)
