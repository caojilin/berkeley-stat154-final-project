...
mean(y_hat == testing$expert_label)
```

```{r}
system.time(fit.decision <- rpart(expert_label ~ SD + CORR + NDAI, 
                   data = train_validation))
y_hat <- predict(fit_randomForest, testing)
mean(y_hat == testing$expert_label)
```
...
logistic_mod1 <- glm(expert_label ~ SD + CORR + NDAI,
                       data = train.cv,
                       family = "binomial")
y_hat_logistic <- predict(logistic_mod1, newdata = testing)
y_hat_logistic <- y_hat_logistic > 0.9
...
roc.curve(testing$expert_label, predict(qda_mod1, testing)$class)

# lda
lda_mod1 <- lda(expert_label ~ SD + CORR + NDAI, 
                     data = train_validation)
roc.curve(testing$expert_label, predict(lda_mod1, testing)$class)
