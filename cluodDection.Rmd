...
mean_accuracy = rep(0, 33)
times = rep(0, 33)
n_tree = c(seq(1,40,2),seq(40,200,20),250,300,350,400)
for(i in 1:33){
  t = system.time(fit_randomForest <- randomForest(expert_label ~ SD + CORR + NDAI, 
                   data = train_validation,
                   ntree = n_tree[i]))
  times[i] = round(t[1], 5)
  y_hat <- predict(fit_randomForest, testing)
  mean_accuracy[i] = mean(y_hat == testing$expert_label)
}

plot(y = mean_accuracy[1:30], x = n_tree[1:30], type="l", 
     xlab = "number of trees", ylab = "mean accuracy",
     main="randomForest hyperparameter convergence and estimation")
```
